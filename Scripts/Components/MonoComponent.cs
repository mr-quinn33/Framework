...
namespace Framework.Scripts.Components
{
    public abstract class MonoComponent<T> : MonoBehaviour, IComponent where T : GameMode<T>, new()
    {
        ...
        {
            return typeof(T).GetMethod("Load", BindingFlags.FlattenHierarchy | BindingFlags.Public | BindingFlags.Static | BindingFlags.InvokeMethod)?.Invoke(null, null) as IGameMode;
        }
    }
}